...
## Workstream - Local build fixes (local - 2026-09-11)

`werror=true` turns three prebuilt third-party link warnings into build failures that nothing in
this tree can fix. Each suppression is scoped as narrowly as the warning allows, so the same
warning raised by fork or upstream code still fails the build.

- **meson.build** - inline tweak (+6 LOC). *Suppresses LNK4099 globally: packman's sentry-native
  and glfw packages ship without their PDBs, so every target that links them warns.*
- **src/d3d9/meson.build** - inline tweak (+6 / -1 LOC). *Suppresses LNK4199 for `d3d9` only. Each
  USD schema plugin contributes a `/DELAYLOAD`, but a pure schema plugin like RemixCategories is
  loaded by USD's plugin system rather than called directly, so `d3d9` imports nothing from it and
  the directive is inert. Kept rather than dropped: it costs nothing and starts working by itself
  if the runtime ever calls into the plugin.*
- **tests/rtx/apps/HydraTestRender/meson.build** - inline tweak (+6 LOC). *Suppresses LNK4098 for
  that one executable, whose prebuilt glfw3 and USD libraries were built against a different CRT.
  Deliberately not global: a CRT mismatch anywhere in the runtime is a real bug and must keep
  failing the build.*
- **scripts/build.ps1** - inline tweak (+4 / -2 LOC). *Calls `vswhere` with `-products *`, without
